...
async function importAll() {
  console.log(`Starting HTTP REST creation for ${users.length} users in Firebase...`);
  let count = 0;
  ...
  for (const u of users) {
    ...
    try {
      ...
      if (resp.ok && resData.localId) {
        firebaseUid = resData.localId;
        console.log(`[${count}/${users.length}] Created user: ${u.email} (UID: ${firebaseUid})`);
      } else if (resData.error && resData.error.message.includes('EMAIL_EXISTS')) {
        console.log(`[${count}/${users.length}] User ${u.email} already registered.`);
      } else {
        console.warn(`[${count}/${users.length}] Notice for ${u.email}:`, resData.error?.message || resData);
      }
  ...
  console.log(`\n🎉 Completed importing all ${users.length} users into Firebase!`);
  process.exit(0);
}
