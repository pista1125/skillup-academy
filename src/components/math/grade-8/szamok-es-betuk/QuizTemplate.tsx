...
export interface QuizTemplateProps {
  ...
  hintText?: string;
  themeColor?: 'orange' | 'blue' | 'emerald' | 'purple' | 'amber' | 'cyan' | 'indigo' | 'violet';
  grade?: number;
  chapterId?: string;
  topicId?: string;
  topicTitle?: string;
...
export function QuizTemplate({
    ...
    if (rawQuestions && rawQuestions.length > 0) {
      ...
      return {
        1: {
          ...
          subtitle: 'Alapfogalmak és egyszerűbb feladatok',
          range: '1 - 10. feladat',
          focus: 'Alapfogalmak',
        ...
        2: {
          ...
          subtitle: 'Összefüggések és gyakorlati feladványok',
          range: '11 - 20. feladat',
  ...
  const hasCheatSheet = (cheatSheet && cheatSheet.length > 0) || (normalizedCheatCards && normalizedCheatCards.length > 0);
    ...
    return (
      ...
      >
          ...
          <div className="flex items-center gap-1.5">
            ...
            {onSwitchToTheory && (
              <Button
                ...
                className="rounded-xl h-7 px-2 text-xs font-bold text-violet-700 bg-violet-50/50 border-violet-200 dark:bg-violet-950/40 dark:text-violet-300 dark:border-violet-800 hover:bg-violet-100 cursor-pointer"
              >
          ...
          <div className="flex items-center gap-1.5">
            ...
            {hasCheatSheet && (
              <Button
                ...
                className="rounded-xl h-7 px-2.5 text-xs font-bold border-violet-300 bg-violet-50/50 text-violet-800 dark:bg-violet-950/40 dark:text-violet-300 dark:border-violet-800 hover:bg-violet-100 cursor-pointer"
              >
        ...
        <div className="text-center max-w-2xl mx-auto mb-2 sm:mb-2.5">
          <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider mb-1 border border-violet-200 dark:border-violet-800 bg-violet-100/80 dark:bg-violet-950/60 text-violet-800 dark:text-violet-300">
            <span>{topicBadge || badgeText || '8. Osztály • Matematika'}</span>
            <span className="opacity-40">•</span>
            <span className="font-extrabold flex items-center gap-1 text-violet-700 dark:text-violet-300">
              <Sparkles className="w-3 h-3 text-violet-500" /> Gyakorló Kvíz
            </span>
          </div>
        ...
        {showCheatSheet && hasCheatSheet && (
          <div className="mb-2.5 p-3 sm:p-4 bg-gradient-to-br from-violet-50 to-indigo-50 dark:from-slate-900 dark:to-slate-850 rounded-xl border-2 border-violet-200 dark:border-violet-900/60 shadow-md animate-in fade-in duration-200">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs sm:text-sm font-black text-violet-900 dark:text-violet-200 flex items-center gap-1.5">
              ...
              <Button
                ...
                className="text-violet-800 dark:text-violet-300 hover:bg-violet-200/50 rounded-lg h-6 px-2 text-xs cursor-pointer"
              >
                Bezárás
              </Button>
            </div>
            {cheatSheet && cheatSheet.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 mb-2">
                {cheatSheet.map((item, idx) => (
                  <div key={idx} className="bg-white/95 dark:bg-slate-800/95 p-2 rounded-lg border border-violet-100 dark:border-slate-700 shadow-xs text-left">
                    <div className="text-[11px] font-black text-violet-600 dark:text-violet-400">{item.topic}</div>
                ...
                {normalizedCheatCards.map((card, idx) => (
                  <div key={card.id || idx} className="bg-white/95 dark:bg-slate-800/95 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-xs text-left space-y-1">
                    <div className="flex items-center gap-1.5 font-bold text-xs text-slate-900 dark:text-slate-100">
                      {card.icon || <Sparkles className="w-3.5 h-3.5 text-violet-600" />}
                    ...
                    {card.content ? (
                      ...
                      <>
                        {card.formula && <div className="text-[11px] font-mono font-bold text-slate-800 dark:text-slate-100 mt-0.5">{card.formula}</div>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-4xl mx-auto">
          {([1, 2, 3] as DifficultyLevel[]).map((level) => {
            ...
            const badgeBg = cfg.badgeBg || (level === 1 ? 'bg-emerald-50 dark:bg-emerald-950/40' : level === 2 ? 'bg-amber-50 dark:bg-amber-950/40' : 'bg-violet-50 dark:bg-violet-950/40');
            const badgeBorder = cfg.badgeBorder || (level === 1 ? 'border-emerald-200 dark:border-emerald-800' : level === 2 ? 'border-amber-200 dark:border-amber-800' : 'border-violet-200 dark:border-violet-800');
            const badgeText = cfg.badgeText || (level === 1 ? 'text-emerald-700 dark:text-emerald-300' : level === 2 ? 'text-amber-700 dark:text-amber-300' : 'text-violet-700 dark:text-violet-300');
            const iconBg = cfg.iconBg || (level === 1 ? 'bg-emerald-600 text-white' : level === 2 ? 'bg-amber-600 text-white' : 'bg-violet-600 text-white');

            return (
              <div
                key={level}
                onClick={() => handleStartLevel(level, gameMode)}
                className="group relative bg-white dark:bg-slate-900 rounded-xl p-3 sm:p-3.5 border-2 border-slate-200/80 dark:border-slate-800 hover:border-violet-500 dark:hover:border-violet-500 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer flex flex-col justify-between overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-bl from-violet-500/10 to-transparent rounded-bl-full pointer-events-none group-hover:scale-110 transition-transform" />
                ...
                <div>
                  ...
                  <h3 className="text-sm sm:text-base font-black text-slate-900 dark:text-white mb-0.5 leading-snug group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
                    {cfg.title}
                  </h3>
                  ...
                  <div className="space-y-1 pt-1.5 border-t border-slate-100 dark:border-slate-800 mb-2">
                    ...
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      <span className="font-bold text-violet-600 dark:text-violet-400 text-right truncate max-w-[130px]" title={cfg.focus}>{cfg.focus}</span>
                    </div>
                  </div>
    ...
    return (
      ...
      >
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-6 border-2 border-slate-200/80 dark:border-slate-800 shadow-xl max-w-lg w-full mx-auto">
          {/* Trophy Badge */}
          <div className="w-14 h-14 sm:w-16 sm:h-16 mx-auto mb-2 rounded-2xl bg-gradient-to-br from-amber-400 to-violet-600 flex items-center justify-center text-white shadow-md shadow-violet-500/20">
          ...
          <div className="grid grid-cols-3 gap-2 bg-slate-50 dark:bg-slate-800/60 p-2.5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 mb-2.5">
            <div>
              ...
              <div className="text-lg sm:text-xl font-black text-violet-600 dark:text-violet-400 font-mono">{score} / {totalQuestions}</div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 mb-2">
            {selectedLevel < 3 && (
              <Button
                ...
                className="flex-1 h-9 sm:h-10 rounded-xl text-xs sm:text-sm font-bold bg-violet-600 hover:bg-violet-700 text-white shadow-sm flex items-center justify-center gap-1.5"
              >
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 pt-2 border-t border-slate-100 dark:border-slate-800">
            ...
            {onSwitchToTheory && (
              <Button
                ...
                size="sm"
                onClick={onSwitchToTheory}
                className="h-7 sm:h-8 px-2.5 rounded-lg text-xs font-bold text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-950/40 flex items-center gap-1"
              >
    ...
    >
                ...
                <div className="flex flex-col gap-2.5">
                  <Card className="border-2 border-slate-200/90 dark:border-slate-800 rounded-2xl shadow-xs overflow-hidden">
                    <div className="px-3.5 py-2 bg-slate-50 dark:bg-slate-850 border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-violet-50 dark:bg-violet-950/60 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-800">
                        {levelConfig.title}
                      </span>
                    </div>
                    ...
                    <CardContent className="p-4 sm:p-5 text-center">
                      ...
                      {currentQuestion.highlightValue && (
                        <div className="inline-block px-6 py-2.5 bg-gradient-to-br from-violet-50 to-indigo-50/60 dark:from-slate-855 dark:to-slate-800 rounded-2xl border-2 border-violet-200/80 dark:border-slate-700 shadow-inner">
                            ...
                            {((currentQuestion.breakdown && currentQuestion.breakdown.length > 0) || (currentQuestion.steps && currentQuestion.steps.length > 0)) && (
                              <div className="flex flex-wrap items-center gap-1.5 pt-1.5 border-t border-slate-200/60 dark:border-slate-700/60">
                                ...
                                {(currentQuestion.breakdown || currentQuestion.steps || []).map((item, bIdx) => (
                                  ...
                                  >
                                    {(item as { label?: string; value?: string }).label || `${bIdx + 1}. lépés`}: <span className="text-violet-600 dark:text-violet-400">{(item as { label?: string; value?: string }).value || String(item)}</span>
                                  </span>
                                ))}
                              </div>
                            )}
                  ...
                  {isAnswerChecked && (
                    <div className="pt-1 animate-in fade-in slide-in-from-bottom-2 duration-200">
                      <Button
                        ...
                        className="w-full h-11 sm:h-12 rounded-xl text-sm font-black bg-slate-900 hover:bg-slate-800 text-white dark:bg-violet-600 dark:hover:bg-violet-700 shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
                      >
                        {currentIndex < levelConfig.questions.length - 1 ? (
                          <>
                            Következő Feladat
                            <ArrowRight className="w-4 h-4 ml-1" />
                            <span className="text-[10px] font-normal text-slate-300 dark:text-violet-100 ml-1.5 opacity-80">(Enter ↵)</span>
                          </>
                        ) : (
                          <>
        ...
        <div className="lg:col-span-3 flex flex-col gap-3">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-3.5 border-2 border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col gap-1.5">
            ...
            >
              {isFullscreen ? (
                <>
                  <Minimize2 className="w-3.5 h-3.5 mr-2 text-violet-600" />
                  Kilépés a teljes képernyőből
                </>
              ) : (
                <>
                  <Maximize2 className="w-3.5 h-3.5 mr-2 text-violet-600" />
      ...
      {showCheatSheet && hasCheatSheet && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border-2 border-violet-300 dark:border-violet-900 shadow-2xl max-w-2xl w-full text-left max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-4">
                {cheatSheet.map((item, idx) => (
                  <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 space-y-1">
                    <div className="text-xs font-bold text-violet-600 dark:text-violet-400">{item.topic}</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-4">
                {normalizedCheatCards.map((card, idx) => (
                  <div key={card.id || idx} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 space-y-1">
                    <div className="flex items-center gap-2 font-bold text-xs text-violet-600 dark:text-violet-400">
