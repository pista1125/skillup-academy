...
export const TheorySection: React.FC<TheorySectionProps> = ({
  ...
  const styles = colorStyles[badgeColor] || colorStyles.cyan;
  return (
...
export interface GeometryFigureCardProps {
  ...
  variant?: 'cyan' | 'blue' | 'indigo' | 'purple' | 'emerald' | 'rose' | 'amber';
  className?: string;
}
...
export const TheoryCallout: React.FC<TheoryCalloutProps> = ({
  ...
  const current = calloutConfig[variant] || calloutConfig.cyan;
...
export const TheoryTemplate: React.FC<TheoryTemplateProps> = (props) => {
  const {
    ...
    pdfFilename = 'geometria-tananyag.pdf',
    badgeText,
    badge,
    topicBadge,
    title,
  ...
  const displayBadge =
    badgeText || badge || topicBadge || '📐 6. Osztály • III. Geometria, szimmetria';
  const displayRule =
    quickRule || (ruleFormula ? { label: ruleTitle || 'Alaptétel', formula: ruleFormula } : undefined);

  const [isDownloading, setIsDownloading] = useState(false);
  const styles = colorStyles[themeColor] || colorStyles.cyan;
          ...
          <div className="flex-1 min-w-0">
            ...
            >
              ...
              <span className="font-extrabold flex items-center gap-1">
                <BookOpen className="w-3.5 h-3.5" /> Tananyag
              </span>
            </div>
