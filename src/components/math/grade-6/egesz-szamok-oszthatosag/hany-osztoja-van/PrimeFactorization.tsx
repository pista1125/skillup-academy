...
import { Calculator, Binary, Hash, ArrowLeft } from 'lucide-react';
...
export function PrimeFactorization({ onBack }: PrimeFactorizationProps) {
    ...
    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
            {onBack && (
                <div className="flex items-center justify-between bg-white/50 p-4 rounded-2xl border border-indigo-100 shadow-sm mb-2">
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 bg-indigo-100 rounded-xl text-indigo-600">
                            <Binary className="w-5 h-5" />
                        </div>
                        <div>
                            <h3 className="font-bold text-base text-slate-800">Prímtényezős felbontás</h3>
                            <p className="text-xs text-slate-500">Számok felbontása prímszámok szorzatára</p>
                        </div>
                    </div>
