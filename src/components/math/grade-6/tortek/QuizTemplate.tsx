...
export function QuizTemplate({
  ...
  const computedTopicId = useMemo(() => {
    ...
    if (t.includes('reciprok')) return 'g6-fractions-mult-reciprocal';
    if (t.includes('osztás törttel') || t.includes('osztas torttel')) return 'g6-fractions-division';
    if (t.includes('tizedes tört') || t.includes('tizedestort')) return 'g6-decimal-fractions-review';
    if (t.includes('összetett') || t.includes('zárójel')) return 'g6-complex-operations';
    if (t.includes('összefoglal') || t.includes('témazáró')) return 'g6-fractions-summary';
      ...
      >
        ...
        <div className="text-center max-w-2xl mx-auto mb-3 sm:mb-4">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider mb-2 border border-amber-200 dark:border-amber-800 bg-amber-100/80 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300">
            <span>{topicBadge || badgeText || '6. Osztály • Matematika'}</span>
                ...
                <div>
                  ...
                  <div className="space-y-1.5 pt-2.5 border-t border-slate-100 dark:border-slate-800 mb-3">
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-bold text-amber-600 dark:text-amber-400 text-right truncate max-w-[140px]" title={cfg.focus}><MathText size="sm">{cfg.focus}</MathText></span>
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 items-start">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {!isAnswerChecked && (
                    <div className="p-2.5 bg-amber-50/60 dark:bg-slate-855/80 rounded-xl border border-amber-200/50 dark:border-slate-800 text-[11px] text-amber-900 dark:text-amber-300 flex items-center gap-1.5 mt-0.5">
                      <Sparkles className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                      <span><MathText size="sm">{currentQuestion.hint || hintText || "💡 Figyelj a helyes szabályokra és összefüggésekre!"}</MathText></span>
                    </div>
                  )}
                </div>
              </div>
