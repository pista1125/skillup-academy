...
export type ThemeColor = 'amber' | 'blue' | 'indigo' | 'violet' | 'purple' | 'emerald' | 'teal' | 'rose' | 'cyan' | 'slate';
...
export const TheorySection: React.FC<TheorySectionProps> = ({
  ...
  const styles = colorStyles[badgeColor] || colorStyles.amber;
  return (
...
export const TheoryTemplate: React.FC<TheoryTemplateProps> = (props) => {
  ...
  const displayBadge = badgeText || badge || topicBadge || '🍕 6. Osztály • II. Törtek';
  const displayRule = quickRule || (ruleFormula ? { label: ruleTitle || 'Alapszabály', formula: ruleFormula } : undefined);
