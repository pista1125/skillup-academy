...
const colorStyles: Record<ThemeColor, {
  ...
  amber: {
    ...
    calloutGradient: 'from-amber-600 via-orange-600 to-amber-700',
  ...
  cyan: {
    ...
    calloutGradient: 'from-cyan-600 via-teal-600 to-cyan-700',
  ...
  slate: {
    ...
    ruleBg: 'bg-slate-50 dark:bg-slate-800/80',
    ruleBorder: 'border-slate-200 dark:border-slate-700',
    ruleText: 'text-slate-700 dark:text-slate-300',
    sectionBadge: 'bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200',
    buttonQuizBorder: 'border-slate-300 dark:border-slate-700',
    buttonQuizBg: 'bg-slate-100 dark:bg-slate-800/60',
    buttonQuizText: 'text-slate-800 dark:text-slate-200',
    buttonQuizHover: 'hover:bg-slate-200 dark:hover:bg-slate-700',
    buttonPdf: 'bg-slate-700 hover:bg-slate-800 text-white',
    calloutGradient: 'from-slate-700 via-slate-800 to-slate-900',
...
export const TheoryCard: React.FC<TheoryCardProps> = ({
  ...
  const variantStyles: Record<string, string> = {
    default: 'bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-700',
    emerald: 'bg-emerald-50/60 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800/60',
    ...
    indigo: 'bg-indigo-50/60 dark:bg-indigo-950/20 border-indigo-200 dark:border-indigo-800/60',
    purple: 'bg-purple-50/60 dark:bg-purple-950/20 border-purple-200 dark:border-purple-800/60',
    rose: 'bg-rose-50/60 dark:bg-rose-950/20 border-rose-200 dark:border-rose-800/60',
    cyan: 'bg-cyan-50/60 dark:bg-cyan-950/20 border-cyan-200 dark:border-cyan-800/60',
    violet: 'bg-violet-50/60 dark:bg-violet-950/20 border-violet-200 dark:border-violet-800/60'
  };

  return (
    <Card className={cn(
      "rounded-2xl border transition-all duration-200 shadow-none overflow-hidden",
      ...
      <CardContent className="p-4 sm:p-5 space-y-3">
        {(title || icon || badge) && (
          <div className="flex items-center justify-between gap-2 border-b border-slate-200/60 dark:border-slate-700/60 pb-2.5">
            ...
            {badge && (
              <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300">
                {badge}
              </span>
            )}
          </div>
        ...
        {figure && (
          <div className="flex items-center justify-center p-3 rounded-xl bg-slate-50/80 dark:bg-slate-900/80 border border-slate-200/60 dark:border-slate-800">
            {figure}
          </div>
        )}
...
// --- LogicFigureCard Component (Visual card with SVG diagram) ---
export interface LogicFigureCardProps {
  title: string;
  subtitle?: string;
  badge?: string;
  figure: React.ReactNode;
  ...
  variant?: 'blue' | 'indigo' | 'purple' | 'emerald' | 'amber' | 'cyan' | 'rose';
  className?: string;
}

export const LogicFigureCard: React.FC<LogicFigureCardProps> = ({
  title,
  subtitle,
  badge,
  figure,
  ...
  const borderColors: Record<string, string> = {
    ...
    indigo: 'border-indigo-200 dark:border-indigo-900/70 hover:border-indigo-400',
    purple: 'border-purple-200 dark:border-purple-900/70 hover:border-purple-400',
    emerald: 'border-emerald-200 dark:border-emerald-900/70 hover:border-emerald-400',
    amber: 'border-amber-200 dark:border-amber-900/70 hover:border-amber-400',
    cyan: 'border-cyan-200 dark:border-cyan-900/70 hover:border-cyan-400',
    rose: 'border-rose-200 dark:border-rose-900/70 hover:border-rose-400'
  };

  return (
    <div className={cn(
      "p-4 sm:p-5 rounded-2xl bg-white dark:bg-slate-900 border-2 transition-all shadow-xs space-y-3.5",
      borderColors[variant] || borderColors.blue,
      className
    )}>
      ...
      <div className="flex items-center justify-center p-3 rounded-xl bg-slate-50/80 dark:bg-slate-850/80 border border-slate-200/60 dark:border-slate-800/80 overflow-x-auto">
        {figure}
      </div>
      ...
      {properties && properties.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
          {properties.map((prop, idx) => (
            <div key={idx} className="p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/60">
              <span className="font-bold text-slate-600 dark:text-slate-300">{prop.label}: </span>
              <span className="font-mono text-slate-800 dark:text-slate-100">{typeof prop.value === 'string' ? <MathText>{prop.value}</MathText> : prop.value}</span>
            </div>
          ))}
        </div>
      )}
...
export interface TheoryCalloutProps {
  type?: 'tip' | 'info' | 'warning' | 'success' | 'example';
  title?: string;
  children: React.ReactNode;
  className?: string;
}
...
export const TheoryCallout: React.FC<TheoryCalloutProps> = ({
  ...
  const configs = {
    tip: {
      ...
      icon: <Lightbulb className="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />,
      titleColor: 'text-amber-900 dark:text-amber-300',
      defaultTitle: 'Tipp & Megfigyelés'
    },
    info: {
      bg: 'bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800',
      icon: <Info className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />,
      titleColor: 'text-blue-900 dark:text-blue-300',
      defaultTitle: 'Fontos Szabály'
    },
    warning: {
      ...
      defaultTitle: 'Gyakori Hiba / Csapda'
    },
    success: {
      ...
      defaultTitle: 'Jegyezd meg jól!'
    },
    example: {
      bg: 'bg-indigo-50 dark:bg-indigo-950/30 border-indigo-200 dark:border-indigo-800',
      icon: <BookOpen className="w-4 h-4 text-indigo-600 dark:text-indigo-400 shrink-0 mt-0.5" />,
      titleColor: 'text-indigo-900 dark:text-indigo-300',
      defaultTitle: 'Kidolgozott Mintapélda'
    }
  };
  ...
  return (
    <div className={cn("p-4 rounded-2xl border flex items-start gap-3", conf.bg, className)}>
      {conf.icon}
      <div className="space-y-1 text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed flex-1">
        <div className={cn("font-bold text-xs uppercase tracking-wider", conf.titleColor)}>
...
export interface TheoryTrapBoxProps {
  ...
  wrongTitle?: string;
  wrongText: React.ReactNode;
  explanation?: React.ReactNode;
  className?: string;
}

export const TheoryTrapBox: React.FC<TheoryTrapBoxProps> = ({
  correctTitle = 'Helyes gondolatmenet',
  correctText,
  wrongTitle = 'Gyakori tévedés / rossz válasz',
  wrongText,
  explanation,
  className
}) => {
  return (
    <div className={cn("rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:p-5 space-y-3", className)}>
      <div className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-slate-500">
        <AlertTriangle className="w-3.5 h-3.5 text-amber-500" />
        Összehasonlítás: Jó vs. Rossz megközelítés
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div className="p-3.5 rounded-xl border border-rose-200 dark:border-rose-900/60 bg-rose-50/50 dark:bg-rose-950/20 space-y-1.5">
          ...
          <div className="text-xs sm:text-sm font-mono text-rose-900 dark:text-rose-200">
            {wrongText}
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-xl border border-emerald-200 dark:border-emerald-900/60 bg-emerald-50/50 dark:bg-emerald-950/20 space-y-1.5">
          ...
          <div className="text-xs sm:text-sm font-mono text-emerald-900 dark:text-emerald-200 font-bold">
            {correctText}
          </div>
        </div>
      </div>
      ...
      {explanation && (
        <div className="text-xs text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-200/50 dark:border-slate-700/50">
          <span className="font-semibold text-slate-700 dark:text-slate-300">Magyarázat: </span>
          {explanation}
        </div>
      )}
    </div>
...
export const TheoryTable: React.FC<TheoryTableProps> = ({
  ...
  return (
    <div className={cn("overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800", className)}>
      <table className="w-full text-left text-xs sm:text-sm">
        ...
        <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 font-mono">
          {(rows || []).map((row, rIdx) => (
...
export const TheoryTemplate: React.FC<TheoryTemplateProps> = ({
  ...
  return (
    <div className="w-full px-2 sm:px-4 py-2 animate-in fade-in duration-300 text-left">
      ...
      >
        {/* Document Header */}
        <div className="border-b border-slate-200 dark:border-slate-800 pb-6 text-left flex flex-col lg:flex-row items-start lg:items-center justify-between gap-5">
          ...
          {quickRule && (
            <div className={cn(
              "p-3.5 sm:p-4 rounded-2xl border text-center shrink-0 w-full lg:w-auto max-w-full lg:max-w-md shadow-sm",
              styles.ruleBg,
              styles.ruleBorder
            )}>
              ...
              <div className={cn(
                "text-xs sm:text-sm font-mono font-bold leading-normal",
                styles.ruleText
              )}>
