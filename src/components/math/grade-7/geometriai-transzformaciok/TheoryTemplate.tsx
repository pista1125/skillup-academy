...
export interface TheoryCardProps {
  ...
  properties?: (string | { label?: string; value: string | React.ReactNode })[];
  variant?:
    | 'default'
    | 'cyan'
    | 'blue'
...
export const TheoryCard: React.FC<TheoryCardProps> = ({
  ...
  const variantStyles: Record<string, string> = {
    ...
    sky: 'border-sky-200 dark:border-sky-900/60 bg-sky-50/40 dark:bg-sky-950/20',
    ...
    teal: 'border-teal-200 dark:border-teal-900/60 bg-teal-50/40 dark:bg-teal-950/20',
  ...
  return (
    ...
    >
      <CardContent className="p-4 space-y-2.5">
        ...
        {figure && (
          <div className="flex items-center justify-center p-2 rounded-xl bg-slate-50/80 dark:bg-slate-900/80 border border-slate-200/60 dark:border-slate-800 min-h-[70px]">
            {figure}
          </div>
        )}
              ...
              <li key={idx}>
                {typeof item === 'string' ? (
                  ...
                  <span>
                    ...
                    {typeof item.value === 'string' ? <MathText>{item.value}</MathText> : item.value}
                  </span>
                )}
              </li>
            ))}
...
// --- GeometryFigureCard Component (Visual geometry card with SVG figure) ---
...
export const GeometryFigureCard: React.FC<GeometryFigureCardProps> = ({
  ...
  const borderColors: Record<string, string> = {
    ...
    sky: 'border-sky-200 dark:border-sky-900/70 hover:border-sky-400',
    ...
    teal: 'border-teal-200 dark:border-teal-900/70 hover:border-teal-400',
  ...
  return (
    <div
      className={cn(
        'rounded-2xl border-2 bg-white dark:bg-slate-850/80 p-4 transition-all space-y-3 flex flex-col justify-between shadow-xs',
        borderColors[variant] || borderColors.teal,
        className
      )}
    >
          ...
          {properties.map((prop, idx) => (
            <div key={idx} className="flex justify-between items-start gap-2 py-0.5">
              ...
              <span className="font-semibold text-slate-800 dark:text-slate-200 text-right">
                {typeof prop.value === 'string' ? <MathText>{prop.value}</MathText> : prop.value}
              </span>
            </div>
          ))}
        </div>
...
export const TheoryCallout: React.FC<TheoryCalloutProps> = ({
  ...
  const calloutConfig: Record<
    string,
    { bg: string; iconBox: string; titleColor: string; defaultIcon: React.ReactNode }
  > = {
    info: {
    ...
    cyan: {
      ...
      defaultIcon: <Compass className="w-4 h-4" />
    },
    sky: {
      bg: 'bg-sky-50/70 dark:bg-sky-950/30 border-sky-200 dark:border-sky-900/60',
    ...
    teal: {
      bg: 'bg-teal-50/70 dark:bg-teal-950/30 border-teal-200 dark:border-teal-900/60',
      iconBox: 'bg-teal-600 text-white',
      titleColor: 'text-teal-900 dark:text-teal-200',
      defaultIcon: <Shapes className="w-4 h-4" />
    }
  };

  const current = calloutConfig[variant] || calloutConfig.teal;

  return (
    <div
      className={cn(
        'p-4 rounded-2xl border text-xs sm:text-sm flex items-start gap-3',
        current.bg,
        className
      )}
    >
      ...
      <div className="space-y-1 flex-1 min-w-0">
        {title && (
          <div className={cn('font-bold', current.titleColor)}>
    ...
    >
      ...
      {traps && traps.length > 0 ? (
        ...
        <ul className="list-disc pl-5 space-y-1.5 text-xs text-slate-600 dark:text-slate-300">
          {items.map((item, idx) => (
            <li key={idx}>
              {typeof item === 'string' ? <MathText>{item}</MathText> : item}
            </li>
          ))}
        </ul>
      ) : children ? (
        ...
        <div className="text-xs text-slate-600 dark:text-slate-300 space-y-1">
          {displayWrong && (
            <div className="flex items-start gap-1.5">
              ...
              <span>
                <strong className="text-rose-600">Gyakori hiba:</strong>{' '}
                {typeof displayWrong === 'string' ? (
                  <MathText>{displayWrong}</MathText>
                ) : (
                  displayWrong
                )}
              </span>
          ...
          {displayCorrect && (
            <div className="flex items-start gap-1.5">
              ...
              <span>
                <strong className="text-emerald-600">Helyes elv:</strong>{' '}
                {typeof displayCorrect === 'string' ? (
                  <MathText>{displayCorrect}</MathText>
                ) : (
                  displayCorrect
                )}
              </span>
          ...
          <thead className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-black">
            <tr>
              {tableHeaders.map((h, i) => (
                <th
                  key={i}
                  className="p-3 border-b border-slate-200 dark:border-slate-700 whitespace-nowrap"
                >
...
export type TheorySection = TheorySectionData;
...
export const TheoryTemplate: React.FC<TheoryTemplateProps> = (props) => {
  const {
    ...
    ruleFormula,
    themeColor = 'teal',
    practiceTitle = 'Készen állsz a geometriai feladványokra?',
    practiceSubtitle = 'Tedd próbára tudásod a 3 szintű kvízben alakzatokkal, ábrákkal és azonnali magyarázatokkal!',
  ...
  const styles = colorStyles[themeColor] || colorStyles.teal;
