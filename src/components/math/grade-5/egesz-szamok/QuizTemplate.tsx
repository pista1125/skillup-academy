          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
            ...
            {onSwitchToTheory && (
              <Button
                variant="ghost"
                onClick={onSwitchToTheory}
                className="h-10 px-3.5 rounded-xl text-xs font-bold text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 flex items-center gap-1.5"
              >
                            ...
                            {((currentQuestion.breakdown && currentQuestion.breakdown.length > 0) || (currentQuestion.steps && currentQuestion.steps.length > 0)) && (
                              <div className="flex flex-wrap items-center gap-1.5 pt-1.5 border-t border-slate-200/60 dark:border-slate-700/60">
                                ...
                                {(currentQuestion.breakdown || currentQuestion.steps || []).map((item, bIdx) => (
                                  ...
                                  >
                                    {(item as { label?: string; value?: string }).label || `${bIdx + 1}. lépés`}: <span className="text-amber-600 dark:text-amber-400">{(item as { label?: string; value?: string }).value || String(item)}</span>
                                  </span>
                                ))}
                              </div>
                            )}
