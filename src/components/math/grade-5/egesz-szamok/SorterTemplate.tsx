...
export function SorterTemplate({
  ...
  const currentConfig = config || levels?.[activeLevel];
          ...
          <div className="flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400 px-1">
            ...
            {selectedItem && (
              <span className="text-amber-600 dark:text-amber-400 animate-pulse">
                Kiválasztva: <strong>{getItemLabel(selectedItem)}</strong> (most kattints a kívánt csoportra!)
              </span>
            )}
          </div>
            ...
            >
              ...
              {selectedItem && (
                <Button
                  ...
                  className="w-full mt-3 h-8 rounded-xl font-bold text-xs bg-amber-600 hover:bg-amber-700 text-white"
                >
                  Ide helyezem: {getItemLabel(selectedItem)}
                </Button>
              )}
            </div>
          );
